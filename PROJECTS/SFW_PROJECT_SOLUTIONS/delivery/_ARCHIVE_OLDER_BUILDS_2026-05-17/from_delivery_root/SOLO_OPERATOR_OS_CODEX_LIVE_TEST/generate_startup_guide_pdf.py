...
story.append(
    p(
        "A plain-English guide for opening, starting, testing, and using the Solo Operator OS live-test build in Codex.",
        styles["Intro"],
    )
)
...
story.append(
    p(
        "It helps one person manage work, business actions, personal admin, ideas, relationships, decisions, follow-ups, and weekly priorities through a structured Codex workspace.",
        styles["Body"],
    )
)
story.append(
...
story.append(p("Open this folder in ChatGPT Codex:", styles["Body"]))
story.append(
    code(
        r"D:\Axis AI - ChatGPT OS\AXIS_OS_CLAUDE_BUILD\PROJECTS\SFW_PROJECT_SOLUTIONS\delivery\SOLO_OPERATOR_OS_CODEX_LIVE_TEST"
    )
)
...
story.append(p("In ChatGPT Codex, type:", styles["Body"]))
story.append(code("AXIS: SOLO START"))
story.append(
    p(
        "Expected result: Codex reads the live-test context, sees setup is already complete, does not repeat first-time onboarding, and offers the main operating choices.",
        styles["Body"],
    )
)
...
for item in [
    "Open the live-test folder in Codex.",
